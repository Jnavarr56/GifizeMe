...
class GifIndex extends React.Component {
    ...
    render () {
        ...
        if (!this.props.masterState.user_gifs.gifs) {
            ...
            return (<div className="fade-in rendered-gifs-container">{this.props.masterState.user_gifs.gifs.map((x, i) => <RenderedGif sendDialog={this.props.sendDialog} createdAt={x.gif_record.created_at} delay={{animationDelay: `${i * .10}s`}} emoji={x.emoji.code} name={x.emoji.name} key={`G${i}`} src={x.blobURL} />)}</div>)
